...
import { Project, Asset, Task, Collection, Token, FileMetadata, UserRole } from '@/backend';
...
export function useCreateTask() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return actor.createTask(projectId, assetId, name, description, status, principal);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
    },
...
export function useMintToken() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error('Actor not initialized');
      const result = await actor.mintToken(projectId, name, description, metadata);
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['tokens', 'project', variables.projectId.toString()] });
      queryClient.invalidateQueries({ queryKey: ['tokens'] });
    },
  });
}
...
export function useFileMetadataByProject(projectId: bigint) {
  ...
  return useQuery<FileMetadata[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      // Fetch all file metadata and filter by projectId client-side
      // since the backend only exposes listFileMetadataByAsset
      // We use a workaround: fetch all assets for the project, then fetch file metadata per asset
      ...
      // Also include project-level file metadata (assetId = null) by checking stored records
      // Since the backend doesn't expose a listFileMetadataByProject, we return what we have
      return allFileMeta;
    },
    enabled: !!actor && !isFetching,
  });
...
export function useUploadFileMetadata() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error('Actor not initialized');
      return actor.storeFileMetadata(projectId, assetId, filename, mimeType, size, hash);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['fileMetadata', 'project', variables.projectId.toString()] });
      queryClient.invalidateQueries({ queryKey: ['fileMetadata'] });
    },
  });
}
...
export function useDeleteFileMetadata() {
  ...
  return useMutation({
    mutationFn: async ({ id, projectId }: { id: bigint; projectId: bigint }) => {
      if (!actor) throw new Error('Actor not initialized');
