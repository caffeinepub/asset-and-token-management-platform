...
export default function CollectionDialog({
  ...
  useEffect(() => {
    if (collection) {
      ...
      setSelectedAssetIds(new Set(collection.assetIds.map((id) => id.toString())));
    } else {
      setName('');
      setDescription('');
              ...
              <ScrollArea className="h-[200px] rounded-md border p-4">
                {assets && assets.length > 0 ? (
                  <div className="space-y-3">
                    {assets.map((asset) => (
                      <div key={asset.id.toString()} className="flex items-center space-x-2">
                        <Checkbox
                          id={`asset-${asset.id.toString()}`}
                          checked={selectedAssetIds.has(asset.id.toString())}
                          onCheckedChange={() => handleAssetToggle(asset.id.toString())}
                        />
                        <label
                  ...
                  <p className="text-sm text-muted-foreground">No assets available in this project.</p>
                )}
              </ScrollArea>
              <p className="text-xs text-muted-foreground">{selectedAssetIds.size} asset(s) selected</p>
            </div>
          </div>
          <DialogFooter>
