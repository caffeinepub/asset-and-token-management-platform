...
import { ArrowLeft, Plus, Loader2, Coins, Calendar, User, Hash, AlertCircle } from 'lucide-react';
import TokenDialog from '@/components/TokenDialog';
import { useProject, useTokensByProject, useMintToken, useMyRole } from '@/hooks/useQueries';
...
export default function TokensPage() {
  ...
  const { data: tokens, isLoading: tokensLoading } = useTokensByProject(BigInt(projectId));
    ...
    <div className="container py-8">
      <div className="mb-8">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2">{project.name} - Tokens</h1>
            <p className="text-muted-foreground mb-4">View and manage tokens minted for this project.</p>
            <Badge variant="secondary">{tokens?.length || 0} tokens</Badge>
          </div>
          {isAdmin && (
            <Button onClick={() => { setMintError(null); setDialogOpen(true); }} size="lg">
      ...
      {!isAdmin && role !== undefined && (
        <Alert className="mb-6">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Only admins can mint tokens. You are viewing tokens in read-only mode.
          </AlertDescription>
        </Alert>
      )}
          ...
          {tokens.map((token) => (
            ...
            >
              <CardHeader>
                ...
                <CardDescription className="line-clamp-3">{token.description || 'No description'}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span className="truncate">By: {token.mintedBy.toString().slice(0, 16)}...</span>
                </div>
                {token.metadata && (
