...
import { ArrowLeft, Plus, Loader2, FileBox, Edit, Trash2, Calendar } from 'lucide-react';
...
import { Asset, UserRole } from '@/backend';
...
export default function AssetsPage() {
  ...
  const { data: assets, isLoading: assetsLoading } = useAssetsByProject(BigInt(projectId));
  ...
  const handleCreateOrUpdate = async (name: string, description: string) => {
    try {
      if (editingAsset) {
        await updateAsset.mutateAsync({ id: editingAsset.id, name, description });
        toast.success('Asset updated successfully');
      } else {
        await createAsset.mutateAsync({ projectId: BigInt(projectId), name, description });
        toast.success('Asset created successfully');
      }
      setDialogOpen(false);
      setEditingAsset(null);
    } catch (error: any) {
      toast.error(error?.message || (editingAsset ? 'Failed to update asset' : 'Failed to create asset'));
    }
  };
