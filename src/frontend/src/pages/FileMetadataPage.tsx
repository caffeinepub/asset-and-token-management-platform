...
function formatBytes(bytes: bigint): string {
  ...
  return `${Number.parseFloat((n / k ** i).toFixed(2))} ${sizes[i]}`;
}
...
export default function FileMetadataPage() {
  ...
  const { data: files, isLoading: filesLoading } = useFileMetadataByProject(
    BigInt(projectId),
  );
  ...
  const handleDelete = async (id: bigint) => {
    if (
      confirm(
        "Are you sure you want to delete this file metadata record? This action cannot be undone.",
      )
    ) {
      try {
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground mb-4">
              Manage file metadata records for this project.
            </p>
